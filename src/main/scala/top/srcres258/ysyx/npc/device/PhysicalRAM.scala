...
class PhysicalRAM(val xLen: Int) extends Module {
    ...
    val readRoutineTimerMax = RegInit(PhysicalRAM.READ_ROUTINE_CLOCK_CYCLES.U(PhysicalRAM.READ_ROUTINE_TIMER_WIDTH.W))
    val writeRoutineTimer = RegInit(0.U(PhysicalRAM.WRITE_ROUTINE_TIMER_WIDTH.W))
    val writeRoutineTimerMax = RegInit(PhysicalRAM.WRITE_ROUTINE_CLOCK_CYCLES.U(PhysicalRAM.WRITE_ROUTINE_TIMER_WIDTH.W))
